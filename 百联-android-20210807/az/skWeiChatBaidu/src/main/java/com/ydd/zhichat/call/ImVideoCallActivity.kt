...
import com.ydd.zhichat.bean.message.XmppMessage.TYPE_END_CONNECT_VIDEO
import com.ydd.zhichat.bean.message.XmppMessage.TYPE_NO_CONNECT_VIDEO
...
import kotlinx.android.synthetic.main.activity_im_video_call.*
import java.text.SimpleDateFormat
import java.util.*

class ImVideoCallActivity : BaseActivity(), View.OnClickListener {
    ...
    companion object {
        ...
        private const val KEY_8 = "firendName"
        ...
        fun start(context: Context,
                  ...
                  token: String,
                  callOrReceive: Int) {
            val starter = Intent(context, ImVideoCallActivity::class.java)
    ...
    private fun sendAnswerMessage() {
        ...
        message.type = XmppMessage.TYPE_CONNECT_VIDEO
        message.content = ""
    ...
    private fun sendCallMessage() {
        if (callOrReceive == CallManager.CALL) {
            val message = ChatMessage()
            message.type = XmppMessage.TYPE_IS_CONNECT_VIDEO
            message.content = InternationalizationHelper.getString("JXSip_invite").toString() + " " + InternationalizationHelper.getString("JX_VideoChat")
            message.fromUserId = mUid
            message.fromUserName = myName
    ...
    private fun sendHangUpMessage() {
        val message = ChatMessage()
        message.type = XmppMessage.TYPE_NO_CONNECT_VIDEO
        ...
        if (ChatMessageDao.getInstance().saveNewSingleChatMessage(mUid, friendUid, message)) { // 更新聊天界面
            ...
            FriendDao.getInstance().updateFriendContent(mUid, friendUid,
                    InternationalizationHelper.getString("JXSip_Canceled").toString() + " " + InternationalizationHelper.getString("JX_VideoChat"),
    ...
    override fun onPause() {
        ...
        if (!isRellyFinish && !isManual) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                if (FloatWindowUtil.checkFloatPermission(this@ImVideoCallActivity)) {
                    startVideoFloatWindow(true)
                } else {
                    ToastUtil.showToast(this@ImVideoCallActivity, "悬浮框未开启")
                }
            } else {
                startVideoFloatWindow(true)
            }
    ...
    override fun onBackPressed() {
        ToastUtil.showToast(this@ImVideoCallActivity, "正在通话中，请先停止通话")
    }
        ...
        when (v?.id) {
            R.id.img_answer -> {//接听
                if (NetworkUtils.isConnected()) {
                    ...
                    ToastUtil.showToast(this@ImVideoCallActivity, "网络不可用")
                    refuseCall()
                }
            }
            R.id.img_refuse -> {//拒接
            ...
            R.id.iv_float -> {//开启悬浮窗
                if (FloatWindowUtil.checkFloatPermission(this@ImVideoCallActivity)) {
                    startVideoFloatWindow()
                } else {
                    ToastUtil.showToast(this@ImVideoCallActivity, "悬浮框未开启")
                }
            }

            R.id.img_switch_camera -> {//切换摄像头
                mRtcEngine!!.switchCamera()
            }
            R.id.small_video_view_container -> {//切换自己和别人的展示框
                if (remoteUid == 0) return//还没连接上视频
                smallIsSelf = !smallIsSelf
                redeployVideo()
            }
        }
    }
    ...
    fun setListener() {
        ...
        small_video_view_container.setOnClickListener(this)
    }
    ...
    private val mRtcEventHandler = object : IRtcEngineEventHandler() { // Tutorial Step 1
        ...
        override fun onFirstRemoteVideoDecoded(uid: Int, width: Int, height: Int, elapsed: Int) {
            runOnUiThread {
                ...
                val remoteSv = RtcEngine.CreateRendererView(baseContext)
                big_video_view_container.addView(remoteSv)
                mRtcEngine?.setupRemoteVideo(VideoCanvas(remoteSv, VideoCanvas.RENDER_MODE_ADAPTIVE, uid))
            }
        }
    ...
    private fun initializeAgoraEngine() {
        ...
        try {
            ...
            mRtcEngine?.setDefaultAudioRoutetoSpeakerphone(true)//默认外放
            val surfaceView = RtcEngine.CreateRendererView(baseContext)
            surfaceView.setZOrderMediaOverlay(true)
            small_video_view_container.addView(surfaceView)
     ...
     * 重新部署视频
     */
    private fun redeployVideo() {
        big_video_view_container.removeAllViews()
        small_video_view_container.removeAllViews()
        val remoteSv = RtcEngine.CreateRendererView(baseContext)
        val localSv = RtcEngine.CreateRendererView(baseContext)
        if (smallIsSelf) {
            localSv.setZOrderMediaOverlay(true)//7.1.1手机必须设置这个属性才能在显示在另一个surfaceView上面
            small_video_view_container.addView(localSv)
            big_video_view_container.addView(remoteSv)
        } else {
            remoteSv.setZOrderMediaOverlay(true)
            small_video_view_container.addView(remoteSv)
            big_video_view_container.addView(localSv)
        }
        mRtcEngine?.setupRemoteVideo(VideoCanvas(remoteSv, VideoCanvas.RENDER_MODE_ADAPTIVE, remoteUid))
        mRtcEngine?.setupLocalVideo(VideoCanvas(localSv, VideoCanvas.RENDER_MODE_ADAPTIVE, 0))
    }
    ...
    private fun stopCallBeforCalling() {
        EventBus.getDefault().post(MessageEventCancelOrHangUp(TYPE_NO_CONNECT_VIDEO, friendUid,
                InternationalizationHelper.getString("JXSip_Canceled").toString() + " " + InternationalizationHelper.getString("JX_VideoChat"), 0))
        JitsistateMachine.isIncall = false
    }
    ...
    private fun stopCallInCalling() {
        ...
        EventBus.getDefault().post(MessageEventCancelOrHangUp(TYPE_END_CONNECT_VIDEO, friendUid,
                InternationalizationHelper.getString("JXSip_Canceled").toString() + " " + InternationalizationHelper.getString("JX_VideoChat"),
                time))
        JitsistateMachine.isIncall = false
    }
    ...
    private var task: TimerTask = object : TimerTask() {
        override fun run() {
            ...
            runOnUiThread {
                if (!JitsistateMachine.isIncall) {
                    mRtcEngine?.leaveChannel()
                    ToastUtil.showToast(this@ImVideoCallActivity, "无人应答")
                }
            }
        }
    }
    ...
    private fun ringPlay() {
        runOnUiThread {
            mediaPlayer = MediaPlayer.create(this@ImVideoCallActivity, R.raw.call)
    ...
    private lateinit var smallSizePreviewLayout: LinearLayout
     ...
     * 开启显示悬浮框
     */
    private fun startVideoFloatWindow(isAutoIntoBack: Boolean = false) {
        initWindow()
    ...
    private fun initWindow() {
        ...
        smallSizePreviewLayout.setOnClickListener {
            //在这里实现点击重新回到Activity
            MyApplication.getInstance().startActivity(Intent(MyApplication.getInstance(), ImVideoCallActivity::class.java))
        }
        //悬浮框触摸事件，设置悬浮框可拖动
        ...
        mRtcEngine?.setupRemoteVideo(VideoCanvas(surfaceView, VideoCanvas.RENDER_MODE_ADAPTIVE, remoteUid))
        // 添加悬浮窗的视图
