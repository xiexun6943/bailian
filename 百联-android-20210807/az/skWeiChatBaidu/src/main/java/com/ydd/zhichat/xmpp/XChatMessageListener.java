...
import com.ydd.zhichat.bean.CodePay;
import com.ydd.zhichat.bean.Contact;
import com.ydd.zhichat.bean.EventLoginStatus;
...
import com.ydd.zhichat.call.CallConstants;
import com.ydd.zhichat.call.MessageEventMeetingInvited;
import com.ydd.zhichat.call.MessageEventSipEVent;
import com.ydd.zhichat.call.MessageHangUpPhone;
...
import com.ydd.zhichat.pay.EventPaymentSuccess;
import com.ydd.zhichat.pay.EventReceiptSuccess;
import com.ydd.zhichat.ui.base.CoreManager;
import com.ydd.zhichat.ui.circle.MessageEventNotifyDynamic;
...
import static com.ydd.zhichat.xmpp.listener.ChatMessageListener.MESSAGE_SEND_SUCCESS;
...
public class XChatMessageListener implements IncomingChatMessageListener {
    ...
    public void newIncomingMessage(EntityBareJid entityBareJid, Message message, Chat chat) {
        ...
        if (fromUserId.equals(toUserId) && !isGroupLiveNewFriendType) {
            if (!resource.equals(MyApplication.MULTI_RESOURCE) && message.getTo().toString().substring(message.getTo().toString().indexOf("/") + 1, message.getTo().length()).equals(MyApplication.MULTI_RESOURCE)) {
                ...
                if (chatMessage.getType() == 26) {
                    ...
                    intent.setAction(com.ydd.zhichat.broadcast.OtherBroadcast.IsRead);
            ...
            if (chatMessage.getFromUserId().equals(mLoginUserId)) {// 其他端发送过来的已读
                ...
                if (msgById != null && msgById.getIsReadDel()) {// 在其他端已读了该条阅后即焚消息，本端也需要删除
                    if (ChatMessageDao.getInstance().deleteSingleChatMessage(mLoginUserId, chatMessage.getToUserId(), packetId)) {
                        ...
                        intent.setAction(com.ydd.zhichat.broadcast.OtherBroadcast.MULTI_LOGIN_READ_DELETE);
                        intent.putExtras(bundle);
                        mService.sendBroadcast(intent);
                    }
                }
        ...
        if (type == XmppMessage.TYPE_INPUT) {
            ...
            intent.setAction(com.ydd.zhichat.broadcast.OtherBroadcast.TYPE_INPUT);
            mService.sendBroadcast(intent);
            return;
        }
